...
main() {
    ...
    Linux)
        ...
        *) log_error "Unsupported architecture ($ARCH) for Linux."; exit 1;;
        esac;;
    Darwin)
        case $ARCH in
        x86_64) SYSTEM="x86_64-darwin";;
        arm64) SYSTEM="aarch64-darwin";; # Darwin uses arm64 for Apple Silicon
        *) log_error "Unsupported architecture ($ARCH) for Darwin/macOS."; exit 1;;
        esac;;
    *) log_error "Unsupported operating system ($OS)."; exit 1;;
    ...
    if [ -z "$LATEST_TAG" ]; then
        log_error "Could not fetch the latest release tag from GitHub."; exit 1
    fi
    ...
    if ! curl -fL "${DOWNLOAD_URL}" -o "${ZIP_PATH}" 2>/dev/null; then
        log_error "Failed to download zip. Check URL or network:"; log_error "  URL: ${DOWNLOAD_URL}"; exit 1
    fi
